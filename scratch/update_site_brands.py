...
# Mapping of file -> (Title, List of (Brand Name, Image File))
activity_brands = {
...
'''
    ...
    # Check if section already exists or replace before footer
    if 'class="activity-brands-section"' in html:
        html = re.sub(r'<!-- SECTION MARQUES SPÉCIFIQUES.*?<!-- FLOATING', section_html + '\n    <!-- FLOATING', html, flags=re.DOTALL)
    else:
        html = html.replace('<!-- FLOATING QUICK CONTACT', section_html + '\n    <!-- FLOATING QUICK CONTACT')

    with open(filepath, "w", encoding="utf-8") as f:
        f.write(html)

print("Updated all 11 subpages with dedicated Activity Brand Showcases!")
